    ...
    try {
      ...
      if (!pool) {
        return res.json({
          database: false,
          counts: null,
          hint: "DATABASE_URL not set or not mysql:// — form data is in ./data JSONL. Use JSONL endpoints below.",
        });
      }
      const [[clRows], [crRows], [svRows]] = await Promise.all([
        pool.query("SELECT COUNT(*) AS n FROM contact_leads"),
        pool.query("SELECT COUNT(*) AS n FROM care_requests"),
        pool.query("SELECT COUNT(*) AS n FROM scheduled_visits"),
      ]);
      res.json({
        database: true,
        counts: {
          contact_leads: Number(clRows[0].n),
          care_requests: Number(crRows[0].n),
          scheduled_visits: Number(svRows[0].n),
        },
      });
    } catch (err) {
      next(err);
    ...
    try {
      ...
      const [rows] = await pool.query(
        `SELECT id, caller_name, care_recipient_name, city_neighborhood, referral_source, message, status, created_at
         FROM contact_leads ORDER BY created_at DESC LIMIT ?`,
        [limit]
      );
      res.json({ rows });
    } catch (err) {
